...
jest.mock('./subSchemas', () => {
  return {
    loadSubSchemas: async () => {
      const [userTypeDefs] = await loadFiles(
        'src/user-service/typeDefs.graphql',
      )
      const userServiceSchema = makeExecutableSchema({
        typeDefs: print(userTypeDefs),
      })

      const [postTypeDefs] = await loadFiles(
        'src/post-service/typeDefs.graphql',
      )
...
describe('gateway schema', () => {
  ...
  describe('stitching', () => {
    test('userService then postService', async () => {
    ...
    test('postService then userService', async () => {
      userQueryMock.mockReturnValue({
        users: () => [
